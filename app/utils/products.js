...
const base_url = isServer() ? process.env.BASE_URL : process.env.NEXT_PUBLIC_BASE_URL
const consumerKey = isServer() ? process.env.CONSUMER_KEY : process.env.NEXT_PUBLIC_CONSUMER_KEY ;
const consumerSecret = isServer() ? process.env.CONSUMER_SECRET : process.env.NEXT_PUBLIC_CONSUMER_SECRET ;

// Add logging to track request origin
const logRequestOrigin = (functionName) => {
    console.log(`${functionName} called from: ${isServer() ? 'Server' : 'Client'} Side`);
  };
