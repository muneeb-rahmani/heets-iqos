...
export async function generateMetadata(props) {
  const data = await getCategoryMetadata('ajman');
...
const Page = async () => {
  ...
  const data = await fetchCategoryAndProducts();
    const rankMathData = await getSEOData(`${config.mainifest.url}/ajman`);
    const jsonLdData = parseRankMathData(rankMathData);
    const metaData = await getCategoryMetadata('ajman');
  return (
    <>
      <Ajman productData={data} metaData={metaData} />
      <script
